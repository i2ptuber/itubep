...
# "segment_%04d_{quality}.ts"). С появлением множественных качеств в ОДНОМ
# торренте (см. publisher.py: все сегменты всех выбранных при публикации
# качеств теперь складываются в единый multi-file торрент, а выбор качества
# зрителем — это просто смена приоритета файлов внутри него, а не отдельный
# торрент) имя сегмента обязано включать суффикс качества, иначе одинаковые
# индексы сегментов разных качеств коллизировали бы по имени. Любой
# .torrent, добавляемый через /bridge/add, ДОЛЖЕН состоять целиком из
# файлов такого вида — иначе это не "видео от ITubeP", а произвольный
# контент, который сайт пытается заставить мост скачать и раздавать
# (см. audit — было критичной находкой).
_QUALITY_LABELS = ("360p", "480p", "720p", "1080p")
_SEGMENT_FILENAME_RE = re.compile(
    r"^segment_\d{4}_(" + "|".join(_QUALITY_LABELS) + r")\.ts$"
)
