...
class TkinterPairingDialog:
    ...
    def _show_brute_force_warning_impl(self, origin: str, attempt_count: int) -> bool:
        ...
        root = tk.Tk()
        root.title("Подозрительная активность")
        ...
        lines = [
            "Сайт прислал много неверных кодов подтверждения подряд:",
            origin,
            "",
            f"Неудачных попыток: {attempt_count}",
            "",
            "Это похоже на попытку подобрать код сопряжения программно,",
            "а не на человека, который просто ошибся при вводе. Обычно",
            "легитимному сайту не нужно присылать код больше пары раз.",
            "",
            "Если вы не пытались сопрячься с этим сайтом сейчас —",
            "рекомендуем заблокировать его.",
        ]
        for line in lines:
        ...
        ttk.Button(btn_frame, text="Игнорировать", command=on_ignore).pack(side="right", padx=5)
        ttk.Button(btn_frame, text="Добавить в блеклист", command=on_block).pack(side="right")
    ...
    def _show_pairing_request_impl(self, origin: str, code: str) -> bool:
        return self._modal(
            title="Запрос доступа к торрент-клиенту",
            lines=[
                f"Сайт запрашивает доступ:",
                origin,
                "",
                "Сможет:",
                "  • Добавлять новые торренты для скачивания",
                "  • Просматривать/удалять ТОЛЬКО те торренты,",
                "    которые сам же добавил",
                "",
                "НЕ сможет:",
                "  • Видеть или трогать торренты других сайтов",
                "  • Видеть список остальных ваших загрузок",
                "",
                f"Код подтверждения: {code}",
                "(введите этот код на странице сайта)",
            ],
            approve_text="Разрешить",
            deny_text="Отклонить",
    ...
    def _modal(self, title: str, lines: list[str], approve_text: str, deny_text: str,
        ...
        if require_code_checkbox:
            ...
            ttk.Checkbutton(
                frame, text="Код введён на странице сайта",
